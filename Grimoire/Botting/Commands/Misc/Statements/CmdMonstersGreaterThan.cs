...
namespace Grimoire.Botting.Commands.Misc.Statements
{
    public class CmdMonstersGreaterThan : StatementCommand, IBotCommand
    {
        ...
        {
            if (World.VisibleMonsters.Count <= int.Parse((string.IsNullOrEmpty(Value1) ? Value1 : (instance.IsVar(Value1)  ? Configuration.Tempvariable[instance.GetVar(Value1)] : Value1))))
            {
                instance.Index++;
            }
